...
-- Pad to sizexsize
function M.Padding(size, direction, padVal)
    local padVal = padVal or 0
    local direction = direction or 'post'
    assert(direction == 'pre' or direction == 'post' or direction == 'center',
        'only accept padding direction with pre | post | center')
    return function(input)
        local indH = input:dim() == 2 and 1 or 2
        local indW = input:dim() == 2 and 2 or 3
        local h = input:size(indH)
        local w = input:size(indW)
        local padH = size - h
        local padW = size - w
        local startH = 1
        local startW = 1
        if padH > 0 or padW > 0 then
            local sizeH = padH > 0 and size or h
            local sizeW = padW > 0 and size or w

            local temp
            if input:dim() == 2 then
                temp = input.new(sizeH, sizeW)
            else
                temp = input.new(input:size(1), sizeH, sizeW)
            end
            if direction == 'center' then
                startH = padH > 0 and math.ceil(padH/2) or startH
                startW = padW > 0 and  math.ceil(padW/2) or startW
            elseif direction == 'pre' then
                startH = padH > 0 and sizeH - h + 1 or startH
                startW = padW > 0 and sizeW - w + 1 or startW
            end
            temp:zero()
                :narrow(indH, startH, h)
                :narrow(indW, startW, w)
                :copy(input)
            input = temp
        end

        return input, torch.Tensor{startH, startW, h, w}
    end
end
