...
local initcheck = argcheck{
    ...
    {name="subset",
     type="string",
     help="LO19 | LN83 | LN84 | LG36 | LN498"},
...
function dataset:__init(...)
    ...
    local imgPathList = paths.concat(
        self.path, 'data', self.subset, 'images.txt'
    )
    local imgPath = readImage(imgPathList)
    local imgClassList = paths.concat(
        self.path, 'annotation', self.subset,
    ...
    for i,line in ipairs(imgPath) do
        ...
        -- if not annotated, there's no bragg spot
        local class = imgClassById[id] or 2
        imgClass[i] = class
    end
    ...
    self.imagePath = pathTensor:narrow(1, startInd, nSplit)
    self.imageClass = classTensor:narrow(1, startInd, nSplit)
...
-- converts a table of samples (and corresponding labels)
-- to a clean tensor
